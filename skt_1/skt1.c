...
void buscarRegistro(Registro *registro, int numRegistros, char *chaveBusca) {
    // Implementação da funcionalidade aqui (EXEMPLO)

    for (int i = 0; i < numRegistros; i++) {
        if (strcmp(registro[i].TecnologiaOrigem, chaveBusca) == 0) {
            printf("Registro encontrado: %s\n", registro[i].TecnologiaOrigem);
